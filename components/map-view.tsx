  ...
  const { clusters, points } = useMemo(() => {
    ...
    wordPoints.forEach((point, i) => {
      ...
      wordPoints.forEach((other, j) => {
        ...
        if (distance(point.lat, point.lng, other.lat, other.lng) < clusterRadius) {
          nearby.push(other)
          processed.add(j)
        }
      })
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
          <div className="max-w-md p-6 bg-background border border-border rounded-lg shadow-lg text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              To use the map view, add your Mapbox access token to <code className="bg-muted px-1 py-0.5 rounded">.env.local</code>:
            </p>
            ...
            <p className="text-xs text-muted-foreground">
              Get a free token at{" "}
              <a href="https://account.mapbox.com/auth/signup/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
                mapbox.com
              </a>
            </p>
          </div>
